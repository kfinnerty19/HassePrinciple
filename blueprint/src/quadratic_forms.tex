...
\[ (x, y) \mapsto x \cdot y := \frac{1}{2}(f(x + y) - f(x) - f(y))\] associated to a quadratic form (see \code{QuadraticMap.associated}) and the definition and characterization of nondegenerate quadratic form (see \code{QuadraticMap.Nondegenerate}).
...
\lean{QuadraticForm.represents_zero_iff_isotropic}
\leanok
\uses{def:represents}
...
By \textcolor{red}{TODO : cite III Prop 2(iv)}, $(c, c) = (-1, c)$, so this can be rewritten as 
...
A computation shows that $d(f_a) = -a d(f)$ and $\epsilon(f_a) = (a, -d(f)) \epsilon (f)$. Combining this with \textcolor{red}{TODO: III Prop 2}, we conclude the result.
\end{proof}
...
Since $\displaystyle \prod_{v \in V} (a, b)_v = \prod_{v \in V} (c, d)_v = 1$ by Theorem \ref{thm:product_formula}, we can deduce from \textcolor{red}{TODO: cite III.2.2 Thm 4} the existence of $x \in \Q^\times$ such that 
...
By \ref{thm:padic_squares_open}, the squares of $\Q_p^\times$ form an open set. Combining this with the approximation theorem \ref{thm:approximation}, we deduce the existence of $x_1, x_2 \in \Q^\times$ such that, denoting $c := h(x_1, x_2)$, one has $c/c_v \in {\Q_v^\times}^2$ for all $v \in S$.
